...
export const useOptimizedCatalogProducts = ({ 
  ...
  const applySearchFilters = (query: any, filters: any) => {
    ...
    if (filters.activeSearchTerm && filters.activeSearchTerm.length >= 1) {
      ...
      query = query.or(`title.ilike.%${searchTerm}%,brand.ilike.%${searchTerm}%,model.ilike.%${searchTerm}%,description.ilike.%${searchTerm}%,seller_name.ilike.%${searchTerm}%,lot_number.ilike.%${searchTerm}%`);
    }
