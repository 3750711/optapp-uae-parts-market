...
const OrderCard: React.FC<OrderCardProps> = ({
  ...
  return <div className={`bg-white rounded-xl shadow-md border hover:shadow-xl transition-all p-4 relative
      ...
      <div className="mt-3">
        ...
        {order.delivery_price_confirm && <div className="flex justify-between items-center text-sm mb-2">
        ...
        <div className="flex flex-wrap gap-2 mb-2">
          ...
          {order.delivery_method && <Badge variant="outline" className="text-gray-600">
              {deliveryMethodLabels[order.delivery_method] || order.delivery_method}
            </Badge>}
          {/* Показываем номер контейнера если есть */}
          {order.container_number && <Badge variant="outline" className="bg-yellow-50 border-yellow-400 text-yellow-700">
        ...
        <div className="text-sm text-gray-500 mb-2">
          <div className="flex items-center justify-between">
            ...
            {(order.seller?.opt_id || order.seller_opt_id) && <span className="text-xs bg-gray-100 px-2 py-1 rounded">
                OPT ID: {order.seller?.opt_id || order.seller_opt_id}
              </span>}
          </div>
        </div>
        
        {order.text_order && order.text_order.trim() !== "" && <div className="text-sm text-gray-600 mb-3 border-t pt-2">
            <span className="font-medium">Доп. информация:</span>
            <p className="mt-1 whitespace-pre-wrap">{order.text_order}</p>
          </div>}
      </div>

      {order.status === 'admin_confirmed' && isSeller && <div className="mb-3">
      ...
      <div className="flex items-center justify-between pt-3 border-t bg-gray-50 -mx-4 px-4 py-3 rounded-b-xl">
        {order.id ? <Link to={`/order/${order.id}`} className="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 border border-input bg-background hover:bg-accent hover:text-accent-foreground h-9 px-4 py-2">
            Подробнее
          </Link> : <span className="text-sm text-gray-400">Недоступно</span>}
      </div>
    </div>;
};
