...
export const validateImportRow = async (
  ...
  if (isNaN(price)) {
    ...
    // Для обычных заказов цена должна быть неотрицательной
    errors.push('Цена товара должна быть нулем или положительным целым числом');
  } else if (price >= 0 && isDebt) {
    // Для долговых заказов любая неотрицательная цена допустима, добавляем предупреждение
    warnings.push('Долговой заказ с неотрицательной ценой');
  }
