...
export const OrderPreviewDialog: React.FC<OrderPreviewDialogProps> = ({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={`${isMobile ? 'max-w-[95vw] max-h-[95vh] m-2' : 'max-w-6xl max-h-[90vh]'} p-0`}>
        <DialogHeader className={`px-${isMobile ? '4' : '6'} py-4 border-b`}>
        ...
        <ScrollArea className={`${isMobile ? 'max-h-[calc(95vh-140px)]' : 'max-h-[calc(90vh-200px)]'}`}>
        ...
        {/* Actions */}
        <div className={`flex ${isMobile ? 'flex-col gap-3 p-4' : 'justify-between p-6'} border-t bg-gray-50`}>
          <Button 
            variant="outline" 
            onClick={handleBack} 
            disabled={isLoading}
          ...
          <Button 
            ...
            disabled={isLoading} 
            className={`bg-green-600 hover:bg-green-700 ${isMobile ? 'w-full touch-target' : ''}`}
          >
            {isLoading ? (
              'Создание заказа...'
            ) : (
