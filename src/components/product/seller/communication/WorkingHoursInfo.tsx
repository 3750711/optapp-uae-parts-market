...
export const WorkingHoursInfo: React.FC<WorkingHoursInfoProps> = ({ isMobile = false }) => {
  ...
  const padding = isMobile ? "p-1.5" : "p-2.5";
  const gap = isMobile ? "gap-1" : "gap-2";

  return (
    <div className={`flex items-center ${gap} text-xs text-gray-600 bg-blue-50 ${padding} rounded border border-blue-100`}>
      <Clock className={`${iconSize} text-blue-600 flex-shrink-0`} />
      <span>{isMobile ? 'Работаем: 9:00-21:00 (UTC+4)' : 'Представители работают: 9:00 - 21:00 (UTC+4)'}</span>
    </div>
  );
};
