...
export const ProductCard: React.FC<ProductCardProps> = ({
  ...
  if (isMobile) {
    return (
      <div className="bg-gradient-to-r from-blue-50 to-green-50 rounded p-1.5 border border-blue-200 shadow-sm mt-1">
        <div className="flex items-start gap-1.5 mb-1">
          <Package className="h-2.5 w-2.5 text-blue-600 mt-0.5 flex-shrink-0" />
          <h4 className="font-medium text-gray-900 text-xs line-clamp-2 leading-tight">
            {truncateTitle(productTitle, 35)}
          </h4>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-gray-600 text-xs">№{lotNumber || '—'}</span>
          <div className="flex items-center gap-1 bg-green-100 px-1 py-0.5 rounded">
            <DollarSign className="h-2.5 w-2.5 text-green-700" />
            <span className="font-bold text-green-800 text-xs">{productPrice}</span>
          </div>
        </div>
      </div>
    );
