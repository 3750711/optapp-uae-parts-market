...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // === MIGRATED TO QSTASH ===
    console.log('📮 [Welcome] Publishing to QStash queue');
    ...
    try {
      ...
      const deduplicationId = generateDeduplicationId('user-welcome', userId);
      
      tgResult = await publishToQueue(
        qstashConfig,
        'welcome',
        { userId, userType: profile.user_type, fullName: profile.full_name, telegramId: profile.telegram_id },
        deduplicationId
      );
