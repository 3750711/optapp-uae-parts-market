-- Improve admin_create_free_order function with separate media tables and enhanced logging
CREATE OR REPLACE FUNCTION public.admin_create_free_order(
  p_title text,
  p_price numeric,
  p_seller_id uuid,
  p_buyer_opt_id text,
  p_brand text DEFAULT '',
  p_model text DEFAULT '',
  p_description text DEFAULT '',
  p_images text[] DEFAULT '{}',
  p_video_url text[] DEFAULT '{}',
  p_delivery_method delivery_method DEFAULT 'self_pickup',
  p_place_number integer DEFAULT 1,
  p_delivery_price_confirm numeric DEFAULT NULL,
  p_text_order text DEFAULT ''
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  v_order_id uuid;
  v_order_number integer;
  v_buyer_id uuid;
  v_seller_name text;
  v_seller_opt_id text;
  sanitized_brand TEXT;
  sanitized_model TEXT;
  image_url TEXT;
  video_url TEXT;
BEGIN
  -- Detailed logging of incoming parameters
  RAISE LOG 'admin_create_free_order called with parameters:';
  RAISE LOG 'p_title: %', p_title;
  RAISE LOG 'p_price: %', p_price;
  RAISE LOG 'p_seller_id: %', p_seller_id;
  RAISE LOG 'p_buyer_opt_id: %', p_buyer_opt_id;
  RAISE LOG 'p_images array length: %', COALESCE(array_length(p_images, 1), 0);
  RAISE LOG 'p_video_url array length: %', COALESCE(array_length(p_video_url, 1), 0);
  RAISE LOG 'p_delivery_method: %', p_delivery_method;
  RAISE LOG 'p_delivery_price_confirm: %', p_delivery_price_confirm;

  -- Only proceed if user is an admin
  IF NOT EXISTS (
    SELECT 1 FROM public.profiles 
    WHERE id = auth.uid() 
    AND user_type = 'admin'
  ) THEN
    RAISE EXCEPTION 'Only administrators can use this function';
  END IF;

  -- Find buyer by OPT_ID
  SELECT id INTO v_buyer_id
  FROM public.profiles
  WHERE opt_id = p_buyer_opt_id
  AND user_type = 'buyer';

  IF v_buyer_id IS NULL THEN
    RAISE EXCEPTION 'Buyer with OPT_ID % not found', p_buyer_opt_id;
  END IF;

  RAISE LOG 'Found buyer with ID: %', v_buyer_id;

  -- Get seller information
  SELECT full_name, opt_id INTO v_seller_name, v_seller_opt_id
  FROM public.profiles
  WHERE id = p_seller_id;

  IF v_seller_name IS NULL THEN
    v_seller_name := 'Unknown Seller';
  END IF;

  RAISE LOG 'Seller info: name=%, opt_id=%', v_seller_name, v_seller_opt_id;

  -- Sanitize brand and model fields like in seller_create_order
  sanitized_brand := COALESCE(NULLIF(TRIM(p_brand), ''), '');
  sanitized_model := COALESCE(NULLIF(TRIM(p_model), ''), '');
  RAISE LOG 'Sanitized brand: %, model: %', sanitized_brand, sanitized_model;

  -- Generate next order number
  SELECT get_next_order_number() INTO v_order_number;
  RAISE LOG 'Generated order number: %', v_order_number;

  -- Insert the free order WITHOUT images and video_url fields
  INSERT INTO public.orders (
    id,
    order_number,
    title,
    price,
    seller_id,
    buyer_id,
    status,
    quantity,
    delivery_method,
    place_number,
    delivery_price_confirm,
    product_id,
    brand,
    model,
    description,
    text_order,
    order_created_type,
    order_seller_name,
    seller_opt_id,
    buyer_opt_id,
    created_at
  ) VALUES (
    gen_random_uuid(),
    v_order_number,
    p_title,
    p_price,
    p_seller_id,
    v_buyer_id,
    'admin_confirmed'::order_status,
    1,
    p_delivery_method,
    p_place_number,
    p_delivery_price_confirm,
    NULL, -- No product_id for free orders
    sanitized_brand,
    sanitized_model,
    p_description,
    p_text_order,
    'free_order'::order_created_type,
    v_seller_name,
    v_seller_opt_id,
    p_buyer_opt_id,
    now()
  )
  RETURNING id INTO v_order_id;

  RAISE LOG 'Free order created successfully with ID: % and number: %', v_order_id, v_order_number;

  -- Insert order images from the images array (like in seller_create_order)
  IF p_images IS NOT NULL AND array_length(p_images, 1) > 0 THEN
    RAISE LOG 'Processing % images', array_length(p_images, 1);
    FOR i IN 1..array_length(p_images, 1) LOOP
      image_url := TRIM(p_images[i]);
      IF image_url IS NOT NULL AND image_url != '' THEN
        INSERT INTO public.order_images (order_id, url, is_primary)
        VALUES (v_order_id, image_url, i = 1);
        RAISE LOG 'Inserted order image: % (primary: %)', image_url, i = 1;
      END IF;
    END LOOP;
  ELSE
    RAISE LOG 'No images to process';
  END IF;

  -- Insert order videos from the videos array (like in seller_create_order)
  IF p_video_url IS NOT NULL AND array_length(p_video_url, 1) > 0 THEN
    RAISE LOG 'Processing % videos', array_length(p_video_url, 1);
    FOR i IN 1..array_length(p_video_url, 1) LOOP
      video_url := TRIM(p_video_url[i]);
      IF video_url IS NOT NULL AND video_url != '' THEN
        INSERT INTO public.order_videos (order_id, url)
        VALUES (v_order_id, video_url);
        RAISE LOG 'Inserted order video: %', video_url;
      END IF;
    END LOOP;
  ELSE
    RAISE LOG 'No videos to process';
  END IF;

  -- Final success log
  RAISE LOG 'admin_create_free_order completed successfully for order ID: %', v_order_id;

  RETURN v_order_id;

EXCEPTION
  WHEN OTHERS THEN
    RAISE LOG 'Error in admin_create_free_order: %', SQLERRM;
    RAISE LOG 'Error context - p_images: %, p_video_url: %', p_images, p_video_url;
    RAISE;
END;
$$;