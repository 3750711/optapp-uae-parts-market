-- Fix seller confirmation trigger to send full order object to edge function

-- Drop and recreate the seller confirmation function to send complete order data
DROP FUNCTION IF EXISTS public.notify_on_seller_confirmation();

-- Create improved function that sends full order object (same format as order creation)
...
BEGIN
  ...
  IF (TG_OP = 'UPDATE' AND 
    ...
    -- Call edge function with full order data (same format as order creation)
    PERFORM
      net.http_post(
