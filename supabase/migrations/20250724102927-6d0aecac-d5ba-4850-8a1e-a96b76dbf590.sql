-- Исправление проблемной строки с 'N/A' в логировании триггера
-- Убираем потенциально проблемную строку с 'N/A' которая вызывает ошибку enum
...
AS $$
BEGIN
  -- Детальное логирование для диагностики (исправлено - убрана проблемная строка с 'N/A')
  RAISE LOG 'notify_on_product_status_changes triggered: operation=%, product_id=%, old_status=%, new_status=%', 
    TG_OP, NEW.id, 
    CASE WHEN TG_OP = 'UPDATE' THEN OLD.status::text ELSE 'INSERT' END, 
    NEW.status;
  ...
  IF ((TG_OP = 'INSERT' AND NEW.status = 'active') OR
    ...
    IF (NEW.status = 'sold' OR EXISTS (
      ...
      BEGIN
        ...
        -- Вызываем Edge Function для отправки уведомления с обработкой ошибок
        BEGIN
          PERFORM
            net.http_post(
        ...
        EXCEPTION
          WHEN OTHERS THEN
            RAISE LOG 'edge function call failed for product %: %', NEW.id, SQLERRM;
            -- Не прерываем создание товара из-за ошибки уведомления
        END;
      END;
    ELSE
