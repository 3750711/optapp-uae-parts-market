-- Add missing fields including images to order notification trigger
-- This will ensure order notifications include photos and other important details
...
BEGIN
  ...
  IF TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND OLD.status IS DISTINCT FROM NEW.status) THEN
    ...
    BEGIN
      ...
      RAISE LOG 'Order notification sent for order % (action: %) with % images', NEW.id, CASE WHEN TG_OP = 'INSERT' THEN 'create' ELSE 'status_change' END, COALESCE(array_length(NEW.images, 1), 0);
    EXCEPTION
      WHEN OTHERS THEN
