-- Migration: Fix Authentication Issues - User Data Sync and First Login Logic (Fixed)
...
-- 7. Create diagnostic function for auth state (with explicit parameter)
CREATE OR REPLACE FUNCTION public.diagnose_auth_state(p_user_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
-- 8. Create overloaded version with default parameter
...
SET search_path = public
AS $$
  SELECT public.diagnose_auth_state(auth.uid());
$$;
