...
-- Create a function to sync existing orphaned users
CREATE OR REPLACE FUNCTION public.sync_orphaned_telegram_users()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  LOOP
    -- Create profile for orphaned telegram user
    INSERT INTO public.profiles (
      id, 
      email, 
      auth_method,
...
-- Execute the sync function to fix existing orphaned users
SELECT public.sync_orphaned_telegram_users();