...
BEGIN
  ...
  -- Handle user_type with proper enum casting
  BEGIN
    user_type_value := COALESCE((NEW.raw_user_meta_data->>'user_type')::user_type, 'buyer'::user_type);
  EXCEPTION
    WHEN others THEN
      user_type_value := 'buyer'::user_type;
  END;
