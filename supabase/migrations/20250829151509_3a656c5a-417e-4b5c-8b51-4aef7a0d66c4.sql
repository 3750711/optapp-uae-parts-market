-- Fix function overloading by dropping all versions and recreating correctly
-- Drop all versions of admin_create_order function (regardless of parameters)
DROP FUNCTION IF EXISTS public.admin_create_order CASCADE;

-- Drop all versions of seller_create_order function (to avoid similar issues)  
DROP FUNCTION IF EXISTS public.seller_create_order CASCADE;

-- Recreate admin_create_order with correct signature and atomic advisory lock logic
...
BEGIN
  ...
  IF NOT pg_try_advisory_xact_lock(12345) THEN
  ...
  RAISE LOG 'admin_create_order: Generated order number % with advisory lock', next_order_number;
  ...
  RAISE LOG 'admin_create_order: Order created successfully with ID: %', created_order_id;
  ...
  IF p_product_id IS NOT NULL THEN
    ...
    SET status = 'sold'
    WHERE id = p_product_id;
    RAISE LOG 'admin_create_order: Product % status updated to sold', p_product_id;
  END IF;
...
-- Recreate seller_create_order with atomic advisory lock logic
...
BEGIN
  ...
  RAISE LOG 'seller_create_order: Generated order number % with advisory lock', next_order_number;
  ...
  RAISE LOG 'seller_create_order: Order created successfully with ID: %', created_order_id;
  ...
  IF p_product_id IS NOT NULL THEN
    ...
    SET status = 'sold'
    WHERE id = p_product_id;
    RAISE LOG 'seller_create_order: Product % status updated to sold', p_product_id;
  END IF;
