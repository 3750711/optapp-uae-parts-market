-- Remove duplicate seller_create_order functions to fix ambiguity error
-- Keep only the correct 18-parameter version

-- First, drop all existing versions of seller_create_order
DROP FUNCTION IF EXISTS public.seller_create_order(text, numeric, integer, uuid, text, text, uuid, text, text, order_status, order_created_type, text, text[], uuid, delivery_method, text, numeric);
DROP FUNCTION IF EXISTS public.seller_create_order(text, numeric, integer, uuid, text, text, uuid, text, text, order_status, order_created_type, text, text[], uuid, delivery_method, text, numeric, text[]);

-- Now create the single correct version with 18 parameters including videos
