-- First, get all admin_create_order function signatures and drop them specifically
DO $$
DECLARE
    func_record RECORD;
BEGIN
    -- Drop all versions of admin_create_order function
    FOR func_record IN 
        SELECT p.oid, p.proname, pg_get_function_identity_arguments(p.oid) as args
        FROM pg_proc p 
        JOIN pg_namespace n ON p.pronamespace = n.oid 
        WHERE n.nspname = 'public' AND p.proname = 'admin_create_order'
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS public.%I(%s) CASCADE', 
                      func_record.proname, func_record.args);
        RAISE LOG 'Dropped function: %(%)', func_record.proname, func_record.args;
    END LOOP;
END $$;

-- Now recreate the correct admin_create_order function
