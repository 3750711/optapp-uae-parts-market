...
BEGIN
  SELECT count(*), min(schema_name || '.' || function_name)
  INTO hardcode_count, example_function
  FROM admin.v_functions_with_hardcoded_urls;
  
  IF hardcode_count > 0 THEN
    RAISE EXCEPTION 'Migration validation failed: Found % function(s) with hardcoded /functions/v1 URLs. Example: %. Please fix all functions to use public.functions_url()', 
      hardcode_count, example_function;
  ELSE
