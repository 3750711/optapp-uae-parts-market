-- Drop existing function and recreate with status field
DROP FUNCTION IF EXISTS public.hybrid_search_products(vector, text, double precision, integer);

-- Create updated hybrid search function with status prioritization
...
BEGIN
  ...
  combined_results AS (
    SELECT DISTINCT
      ...
      COALESCE(sr.status, ekr.status) as status
  ...
  ORDER BY 
    -- First priority: active status (опубликован)
    CASE WHEN sr.status = 'active' THEN 1 ELSE 2 END ASC,
    -- Second priority: exact matches
    sr.exact_match_score DESC,
    -- Third priority: semantic similarity
    sr.similarity DESC,
    -- Fourth priority: title length (more specific titles first)
    length(sr.title) ASC
  LIMIT match_count;
END;
$$;