...
-- Исправляем дублирование видео в заказах
...
BEGIN
  ...
  -- Вставляем заказ БЕЗ video_url поля (оставляем пустым для новых заказов)
...
BEGIN
  ...
  -- Вставляем заказ БЕЗ video_url поля
  INSERT INTO public.orders (
    order_number,
    title,
    price,
...
-- Исправляем заказ №7547 - очищаем поле video_url
UPDATE public.orders 
SET video_url = '{}' 
WHERE order_number = 7547;

-- Исправляем другие заказы с дублированными видео
UPDATE public.orders 
SET video_url = '{}' 
WHERE id IN (
