-- Create new trigger for registered order notifications using existing send-telegram-notification function
...
BEGIN
  ...
  IF (TG_OP = 'INSERT' AND NEW.status = 'processed') OR 
    ...
    -- Call the existing send-telegram-notification function with registered action
    PERFORM
      net.http_post(
...
DROP FUNCTION IF EXISTS public.trigger_notify_registered_order_status();