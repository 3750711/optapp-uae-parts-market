-- Update function to notify registered order status changes with service role token
...
BEGIN
  ...
  IF (TG_OP = 'INSERT' AND NEW.status = 'processed') OR 
    ...
    RAISE LOG 'Triggering registered order notification for order % with status %', NEW.id, NEW.status;
    
    -- Call the registered order notification edge function with service role token
    PERFORM
      net.http_post(
        url:='https://vfiylfljiixqkjfqubyq.supabase.co/functions/v1/send-registered-order-notification',
        headers:='{"Content-Type": "application/json", "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZmaXlsZmxqaWl4cWtqZnF1YnlxIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc0NDg5MTAyNSwiZXhwIjoyMDYwNDY3MDI1fQ.v1ILJWaUSSUJ17lKOJpVsZvhLAjhUYCCEBzf6Cm3AKs"}'::jsonb,
    ...
    RAISE LOG 'Successfully sent registered order notification HTTP request for order %', NEW.id;
  END IF;
  
  RETURN NEW;
EXCEPTION
  WHEN OTHERS THEN
    RAISE LOG 'Error in registered order notification trigger for order %: %', NEW.id, SQLERRM;
