...
-- Step 1: Drop existing policy that depends on the function
...
-- Step 3: Create updated function with proper parameters
...
BEGIN
  ...
  IF current_profile.user_type IS DISTINCT FROM p_user_type THEN
  ...
  IF current_profile.verification_status IS DISTINCT FROM p_verification_status THEN
    RAISE EXCEPTION 'Cannot change verification status. Contact administrator.';
  END IF;
  
  -- Prevent is_trusted_seller changes
  IF current_profile.is_trusted_seller IS DISTINCT FROM p_is_trusted_seller THEN
...
-- Step 4: Add specific RLS policy for Telegram registration completion
...
-- Step 5: Recreate the main update policy with updated logic
