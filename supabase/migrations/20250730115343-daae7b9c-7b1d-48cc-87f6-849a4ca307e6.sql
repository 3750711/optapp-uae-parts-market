-- Fix trigger timing issue: change from BEFORE UPDATE to AFTER UPDATE
-- This prevents the "tuple already modified" error when accepting price offers
...
-- Also need to update the function to handle AFTER UPDATE logic
...
BEGIN
  ...
  IF NEW.status = 'accepted' AND OLD.status != 'accepted' THEN
    ...
    -- Since this is now AFTER UPDATE, we need to update the order_id separately
