...
-- Обновляем функцию admin_create_order для работы с видео из Cloudinary
...
BEGIN
  ...
  -- Сохраняем видео в отдельную таблицу order_videos
...
-- Обновляем функцию seller_create_order для работы с видео из Cloudinary
...
-- Обновляем функцию create_user_order для работы с видео из Cloudinary
...
-- Удаляем функцию transfer_product_videos_to_order так как она больше не нужна
DROP FUNCTION IF EXISTS public.transfer_product_videos_to_order(uuid, uuid);
